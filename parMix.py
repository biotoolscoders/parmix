...
def get_parser():
    parser = argparse.ArgumentParser(description='PedMix_More')
    parser.add_argument('-c', dest='child', help='Number of Children', type = int, required=True)
    parser.add_argument('-p', dest='phasing', help='Phasing Error Invovled. 1: Without Phasing Error, Run Independent; 2: With Phasing Error', type = int, required=True, choices=[1, 2])
    ...
    parser.add_argument('-A', dest='Alleles_A', help='Allele Frenqucies File Path of Population A', required=True)
    parser.add_argument('-B', dest='Alleles_B', help='Allele Frenqucies File Path of Population B', required=True)
    ...
    parser.add_argument('-e', dest='error', help='Genotype Error Rate (Default is 10E-8)', type = float, default = 0.00000001)
    return parser
...
if(args.phasing == 1):
    ...
    with open('./Result1RecomInd.dat', 'w') as fw:
    ...
    with open('./Result2AnceInd.dat', 'w') as fw:
        for i in range(length):
            fw.write('%s ' % data2[i])
    time2 = time.time()
    ...
    with open('./Result3GenoInd.dat', 'w') as fw:
        for i in range(length):
            fw.write('%s ' % data3[i])
    time2 = time.time()
    ...
    with open('./Result1Recom.dat', 'w') as fw:
        for i in range(length):
    ...
    with open('./Result2Ance.dat', 'w') as fw:
        for i in range(length):
            fw.write('%s ' % data2[i])
    time2 = time.time()
    print('Model 2 Finished')
    ...
    with open('./Result3Geno.dat', 'w') as fw:
        for i in range(length):
            fw.write('%s ' % data3[i])
    time2 = time.time()
    print('Model 3 Finished')
