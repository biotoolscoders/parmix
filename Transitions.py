...
def forwardbackward(probability, phasingerror,GenoC, Nc,number, model, t2p, Comodel, LDF1, LDF2,LDF3, LDF4, LD1, LD2, LD3, LD4):
    T2P = t2p
    if(model == 0):
        ...
        if(LDF4 == 0):
            T1[0][0] = math.log(LD4)
            T1[0][1] = math.log(1-LD4)
            T1[1][0] = math.log(LD4)
            T1[1][1] = math.log(1-LD4)
        elif(LDF4 == 1):
            T1[0][0] = math.log(float(LD4[0]))
            T1[0][1] = math.log(float(LD4[1]))
            T1[1][0] = math.log(float(LD4[2]))
            T1[1][1] = math.log(float(LD4[3]))

        T2 = numpy.ones((2,2), dtype = object)
        if(LDF3 == 0):
            T2[0][0] = math.log(LD3)
            T2[0][1] = math.log(1-LD3)
            T2[1][0] = math.log(LD3)
            T2[1][1] = math.log(1-LD3)
        elif(LDF3 == 1):
            T2[0][0] = math.log(float(LD3[0]))
            T2[0][1] = math.log(float(LD3[1]))
            T2[1][0] = math.log(float(LD3[2]))
            T2[1][1] = math.log(float(LD3[3]))

        T3 = numpy.ones((2,2), dtype = object)
        if(LDF2 == 0):
            T3[0][0] = math.log(LD2)
            T3[0][1] = math.log(1-LD2)
            T3[1][0] = math.log(LD2)
            T3[1][1] = math.log(1-LD2)
        elif(LDF2 == 1):
            T3[0][0] = math.log(float(LD2[0]))
            T3[0][1] = math.log(float(LD2[1]))
            T3[1][0] = math.log(float(LD2[2]))
            T3[1][1] = math.log(float(LD2[3]))

        T4 = numpy.ones((2,2), dtype = object)
        if(LDF1 == 0):
            T4[0][0] = math.log(LD1)
            T4[0][1] = math.log(1-LD1)
            T4[1][0] = math.log(LD1)
            T4[1][1] = math.log(1-LD1)
        elif(LDF1 == 1):
            T4[0][0] = math.log(float(LD1[0]))
            T4[0][1] = math.log(float(LD1[1]))
            T4[1][0] = math.log(float(LD1[2]))
            T4[1][1] = math.log(float(LD1[3]))
        T = numpy.concatenate((T1,T2, T3, T4))
    if(Comodel == 0):
        ...
        for i in range(Nc):
            T4 = numpy.ones((2,2), dtype = object)
            T4[0][0] = math.log(1-phasingerror)
            T4[0][1] = math.log(phasingerror)
            T4[1][0] = math.log(phasingerror)
            T4[1][1] = math.log(1-phasingerror)
            T = numpy.concatenate((T,T4))
